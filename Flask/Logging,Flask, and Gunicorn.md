...
Posted  2 years ago  by  Thomas Stringer
...
![Flask logging withing Gunicorn](https://trstringer.com/images/flask-gunicorn-1.png)
...
![More logging output](https://trstringer.com/images/flask-gunicorn-2.png)

Hmmm… looks like only our  _error_  and  _critical_  log messages came through, but not  _debug_,  _info_, and  _warning_  messages.
...
![More logging from Flask](https://trstringer.com/images/flask-gunicorn-3.png)

And if we specify a higher logging level, such as “warning”, we only get the warning (and above) logging messages from both Gunicorn  _and_  our Flask application:

```bash
$ gunicorn --workers=0 --bind=0.0.0.0:8000 --log-level=warning app:app
```

![Last image of logging](https://trstringer.com/images/flask-gunicorn-4.png)
